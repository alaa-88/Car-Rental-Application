...
void add_cars(int &num,Car cars[100]);
void add_bus(int &num,Bus bus[100]);
void add_scooters(int &num,Scooter scooter[100]);
...
MyExcelFile.open("customers.csv");

        MyExcelFile<<"id"<<string(13, ' ')<<"name"<<string(11, ' ')<<"location"<<string(7, ' ')<<"gender"<<string(9, ' ')<<"age"<<string(12, ' ')<<"no orders"<<string(12, ' ')<<"password";
        MyExcelFile<<"\n"<<string(100, '_');
        for(int i=0;i<=num_customers;i++){
            MyExcelFile<<"\n"<<customers[i].id<<string(13, ' ');
            MyExcelFile<<customers[i].name<<string((15-customers[i].name.length()), ' ');
            MyExcelFile<<customers[i].find_location()<<string(14, ' ');
            MyExcelFile<<customers[i].gender<<string(14, ' ');
            MyExcelFile<<customers[i].age<<string(13, ' ');
            MyExcelFile<<customers[i].no_order<<string(13, ' ');
            MyExcelFile<<customers[i].get_pass();


            }}
    else if(case_num==2){
     ofstream MyExcelFile;
MyExcelFile.open("drivers.csv");

        MyExcelFile<<"id"<<string(13, ' ')<<"name"<<string(11, ' ')<<"location"<<string(7, ' ')<<"gender"<<string(9, ' ')<<"age"<<string(12, ' ')<<"experience"<<string(5, ' ')<<"rate"<<string(4, ' ')<<"available"<<string(12, ' ')<<"password";
        MyExcelFile<<"\n"<<string(110, '_');
        for(int i=0;i<=num_drivers;i++){
            MyExcelFile<<"\n"<<drivers[i].id<<string(13, ' ');
            MyExcelFile<<drivers[i].name<<string((15-drivers[i].name.length()), ' ');
            MyExcelFile<<drivers[i].find_location()<<string(14, ' ');
            MyExcelFile<<drivers[i].gender<<string(14, ' ');
            MyExcelFile<<drivers[i].age<<string(13, ' ');
            MyExcelFile<<drivers[i].experience<<string(13, ' ');
            MyExcelFile<<drivers[i].rate<<string(6, ' ');
            MyExcelFile<<drivers[i].driver_decision()<<string(6, ' ');
            MyExcelFile<<drivers[i].get_pass();
            }
        }
    ...
    ofstream MyExcelFile;
MyExcelFile.open("cars.csv");
        MyExcelFile<<"model"<<string(3, ' ')<<"id"<<string(3, ' ')<<"driver_name"<<string(3, ' ')<<"location"<<string(3, ' ')<<"color"<<string(3, ' ')<<"brand"<<string(3, ' ')<<"seats"<<string(3, ' ')<<"available"<<string(3, ' ')<<"price_km/h"<<string(3, ' ')<<"wifi"<<string(3, ' ')<<"airConditioning"<<string(3,' ');
        MyExcelFile<<"\n"<<string(110, '_');
        for(int i=0;i<=num_cars;i++){
            MyExcelFile<<"\n"<<cars[i].model<<string(5, ' ');
            MyExcelFile<<cars[i].id<<string(5, ' ');
            MyExcelFile<<cars[i].driver_name<<string((15-cars[i].driver_name.length()), ' ');
            MyExcelFile<<cars[i].location<<string(7, ' ');
            MyExcelFile<<cars[i].color<<string(5, ' ');
            MyExcelFile<<cars[i].brand<<string(7, ' ');
            MyExcelFile<<cars[i].seats<<string(7, ' ');
            MyExcelFile<<cars[i].is_available<<string(10, ' ');
            MyExcelFile<<cars[i].price_kilo<<string(10, ' ');
            MyExcelFile<<cars[i].wifi_available<<string(7, ' ');
            MyExcelFile<<cars[i].airConditioning_available;
            }
        }
    else if(case_num==4){
    ofstream MyExcelFile;
MyExcelFile.open("buses.csv");
        MyExcelFile<<"model"<<string(4, ' ')<<"id"<<string(4, ' ')<<"driver_name"<<string(4, ' ')<<"location"<<string(4, ' ')<<"color"<<string(4, ' ')<<"brand"<<string(4, ' ')<<"seats"<<string(4, ' ')<<"available"<<string(4, ' ')<<"price_km/h"<<string(4, ' ')<<"time"<<string(4, ' ')<<"station"<<string(4,' ');
        MyExcelFile<<"\n"<<string(115, '_');
        for(int i=0;i<=num_bus;i++){
            MyExcelFile<<"\n"<<bus[i].model<<string(6, ' ');
            MyExcelFile<<bus[i].id<<string(6, ' ');
            MyExcelFile<<bus[i].driver_name<<string((15-bus[i].driver_name.length()), ' ');
            MyExcelFile<<bus[i].location<<string(8, ' ');
            MyExcelFile<<bus[i].color<<string(7, ' ');
            MyExcelFile<<bus[i].brand<<string(7, ' ');
            MyExcelFile<<bus[i].seats<<string(7, ' ');
            MyExcelFile<<bus[i].is_available<<string(10, ' ');
            MyExcelFile<<bus[i].price_kilo<<string(10, ' ');
            MyExcelFile<<bus[i].time<<string(7, ' ');
            MyExcelFile<<bus[i].station;
            }
        }
        else if(case_num==5){
        ofstream MyExcelFile;
MyExcelFile.open("scooters.csv");
        MyExcelFile<<"model"<<string(6, ' ')<<"id"<<string(3, ' ')<<"driver_name"<<string(5, ' ')<<"location"<<string(3, ' ')<<"color"<<string(4, ' ')<<"brand"<<string(7, ' ')<<"seats"<<string(3, ' ')<<"available"<<string(3, ' ')<<"price_km/h"<<string(3, ' ');
        MyExcelFile<<"\n"<<string(100, '_');
        for(int i=0;i<=num_scooters;i++){
            MyExcelFile<<"\n"<<scooter[i].model<<string(7, ' ');
            MyExcelFile<<scooter[i].id<<string(5, ' ');
            MyExcelFile<<scooter[i].driver_name<<string((15-scooter[i].driver_name.length()), ' ');
            MyExcelFile<<scooter[i].location<<string(9, ' ');
            MyExcelFile<<scooter[i].color<<string(7, ' ');
            MyExcelFile<<scooter[i].brand<<string(8, ' ');
            MyExcelFile<<scooter[i].seats<<string(7, ' ');
            MyExcelFile<<scooter[i].is_available<<string(10, ' ');
            MyExcelFile<<scooter[i].price_kilo;
            }
        }
    }
...
fstream strm;
        switch(n){
            case 1:{
                strm.open("customers.csv", ios_base::in);
                if (strm.is_open()) {
                    char arr[100000];

                    while (!strm.eof()) {
                        strm.getline(arr, 100000 , ',');
                        cout << arr ;
                    }
        ...
        case 2:{
                strm.open("drivers.csv", ios_base::in);
                if (strm.is_open()) {
                    char arr[100000];
    ...
    case 3:{
                strm.open("cars.csv", ios_base::in);
                if (strm.is_open()) {
                    char arr[10000];

                    while (!strm.eof()) {
                        strm.getline(arr, 10000 , ',');
                        cout << arr ;
                    }
    ...
    case 4:{
                strm.open("buses.csv", ios_base::in);
                if (strm.is_open()) {
                    char arr[100000];
    ...
    case 5:{
                strm.open("scooters.csv", ios_base::in);
                if (strm.is_open()) {
                    char arr[100000];
